...
module.exports.run = async (lanisBot, message, args) => {
    ...
    lanisBot.database.get(`SELECT * FROM stats WHERE ID = '${userID}'`, (error, row) => {
        ...
        lanisBot.database.run(`UPDATE stats SET vialsStored = vialsStored + 1 WHERE ID = '${userID}';`)
        lanisBot.channels.get(Channels.vialLogs.id).send(`Vial added for <@${userID}> by ${message.member.toString()}. (${row.vialsStored + 1} remaining vials)`)
    })
}
